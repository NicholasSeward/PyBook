> NOTE: Adapted from [*Think Python*, 3rd edition](https://allendowney.github.io/ThinkPython/) by Allen B. Downey, [Chapter 16: Classes and Objects](https://github.com/AllenDowney/ThinkPython/blob/v3/chapters/chap16.ipynb). Provided here for convenience.
...
We'll write methods that create and modify these objects, and we'll use the `turtle` module to draw them.
...
So we'll use the `turtle` module to draw lines on the screen.

As we did in [Chapter 4](section_turtle_module), we'll use `make_turtle` to clear the canvas so the turtle can draw.
To draw lines, we'll use two new functions from the `turtle` module:
...
from copy import deepcopy
box4 = deepcopy(box3)
print(box3.translate(50, 30))
print(box4.grow(100, 60))
```
...
Include special methods `__init__` and a `__str__`, and a method called `draw` that uses `turtle` functions to draw the circle.
