> NOTE: Adapted from [*Think Python*, 3rd edition](https://allendowney.github.io/ThinkPython/) by Allen B. Downey, [Chapter 4: Functions and Interfaces](https://github.com/AllenDowney/ThinkPython/blob/v3/chapters/chap04.ipynb). Provided here for convenience.

# Functions and Interfaces

This chapter introduces a module called `turtle`, which allows you to create simple drawings by giving instructions to an imaginary turtle.
...
Now we can use the functions defined in the module, like `forward`, plus a small `make_turtle` helper that clears the canvas.

```turtle
def make_turtle():
    """Start a blank turtle canvas (stdlib turtle)."""
    from turtle import clearscreen, shape, speed
    clearscreen()
    shape('classic')
    speed('fastest')
...
We will use functions defined in the `turtle` module many times, so it would be nice if we did not have to write the name of the module every time.
...
This version of the import statement imports `forward` from the `turtle` module (and we define `make_turtle` as a helper) so we can call them like this.
...
`turtle` provides two other functions we'll use, called `left` and `right`.
...
def make_turtle():
    ...
    speed('fastest')
make_turtle(delay=0)
print(polygon(n=20, length=9))
print(arc(radius=70, angle=70))
circle(radius=10)
```
...
![](https://github.com/AllenDowney/ThinkPython/raw/v3/turtle_pie.png)
...
print(jump(-80))
size = 40
print(draw_pie(5, size))
print(jump(2 * size))
print(draw_pie(6, size))
print(jump(2 * size))
print(draw_pie(7, size))
```
...
![](https://github.com/AllenDowney/ThinkPython/raw/v3/turtle_flower.png)
...
def make_turtle():
    ...
    shape('classic')
    speed('fastest')
from turtle import render
turtle = make_turtle(auto_render=False)
print(jump(-60))
n = 7
radius = 60
angle = 60
print(flower(n, radius, angle))
print(jump(120))
n = 9
radius = 40
angle = 85
...
There are several modules like `turtle` in Python, and the one we used in this chapter has been customized for this book.
