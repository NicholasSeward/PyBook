> NOTE: Adapted from [*Think Python*, 3rd edition](https://allendowney.github.io/ThinkPython/) by Allen B. Downey, [Chapter 13: Files and Databases](https://github.com/AllenDowney/ThinkPython/blob/v3/chapters/chap13.ipynb). Provided here for convenience.
...
```py
print(download('https://github.com/AllenDowney/ThinkPython/raw/v3/photos.zip'))
```
...
Path('memo.txt').write_text('Writing programs (or programming) is a very creative and rewarding activity.\n')
import os
print(os.getcwd())
...
print(writer.write(str(num_years)))
print(writer.write(str(num_camels)))
print(writer.close())
```
...
```py
print(open('camel-spotting-book.txt').read())
```
...
num_camels = 23
writer = open('camel-spotting-book.txt', 'w')
print(writer.write(f'Years of observation: {num_years}\n'))
print(writer.write(f'Camels spotted: {num_camels}\n'))
print(writer.close())
```
...
```py
config = {'photo_dir': 'photos', 'data_dir': 'photo_info', 'extensions': ['jpg', 'jpeg']}
import yaml
config_filename = 'config.yaml'
writer = open(config_filename, 'w')
print(yaml.dump(config, writer))
print(writer.close())
```
...
config_readback = yaml.safe_load(reader)
print(config_readback)
print(config is config_readback)
```
...
print(os.getcwd())
config = {'photo_dir': 'photos', 'data_dir': 'photo_info', 'extensions': ['jpg', 'jpeg']}
print(os.makedirs(config['data_dir'], exist_ok=True))
```
...
print(key)
anagram_list = db[key]
print(anagram_list.append(word))
db[key] = anagram_list
```
...
old = 'photos'
new = 'images'
print(replace_all(old, new, source_path, dest_path))
```
